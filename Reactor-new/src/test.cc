...
void Accept_cb(Reactor &R , Server_Base::Ptr server)
{
...
void Read_cb(Reactor &R ,Server_Base::Ptr server)
{
...
void Write_cb(Reactor &R ,Server_Base::Ptr server)
{
    int clientfd  = R.Get_Now_Event().data.fd;
...
int main()
{
    ...
    for(int i=0;i<SERVER_NUM;i++)
    {
        ...
        servers[i]->callback.Accept_cb = std::bind(Accept_cb,std::ref(R),servers[i]);
        servers[i]->callback.Read_cb = std::bind(Read_cb,std::ref(R),servers[i]);
        servers[i]->callback.Write_cb = std::bind(Write_cb,std::ref(R),servers[i]);
        servers[i]->callback.Exit_cb = std::bind(Exit_cb);
        servers[i]->Init_Epoll_Fd();
        R.Add_Reactor(servers[i]->Get_Epoll_Fd(),sock,EPOLLIN);
        int timerfd = servers[i]->Start_Timer();
        R.Add_Reactor(servers[i]->Get_Epoll_Fd(), timerfd, EPOLLIN | EPOLLET);
        ths[i] = std::thread(&Reactor::Event_Loop,&R,std::ref(servers[i]),servers[i]->Get_Epoll_Fd(),-1);
